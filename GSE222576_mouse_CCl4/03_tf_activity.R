# GSE222576 - Step 3: 상위 전사인자(TF) 활성 추론 (decoupleR + CollecTRI, mouse)
# GSE135251과 동일한 run_tf_activity() 함수를 재사용 (플랫폼이 달라도 정규화된 발현행렬만 있으면 동일 절차)
...
# probeset(transcript cluster) -> gene symbol, 중복 probeset은 limma::avereps로 평균 (표준 관행)
...
message(sprintf("시점간(Control->10w) 유의한 TF (BH padj<0.05): %d / %d", n_sig, nrow(tf_stats)))

if (n_sig >= 5) {
  top_tf = tf_stats %>% filter(padj < 0.05) %>% slice_head(n = 30) %>% pull(TF)
  heatmap_title = sprintf("TF activity across CCl4 time course (BH padj<0.05, n=%d)", length(top_tf))
} else {
